...
def audit_startup(policy, raw, record):
    ...
    modes = {'E': 'all_optimized', 'E-Least': 'all_optimized', 'E-Weighted': 'all_optimized', 'E-NoOpt': 'none_optimized'}
...
def expected_scheduler(policy):
    mapping = {'Static-512':'least_waiting', 'E':'estimated_completion',
               'E-NoOpt':'estimated_completion', 'E-Least':'least_waiting', 'E-Weighted':'capacity_weighted'}
    if policy not in mapping:
        raise ValueError('Unsupported current publication policy')
    return mapping[policy]


def trace_label(trace, name, time_tick, duration):
    if time_tick >= ticks(duration):
        return None
    if name == 'mixture':
        return 'Mixed'  # no single arrival type is asserted for a mixed minute
    width = int(name.removeprefix('majority'))
    if width not in (15,30,60):
        raise ValueError('Unsupported trace construction')
    return trace['bins'][str(width)][time_tick//ticks(width)]['resolved']
...
def audit_run(result, row, directory, expected, campaign):
    ...
    window = run['window_s']
    short, long = effective['short_output_tokens'], effective['long_output_tokens']
    original_input, duration = effective['input_tokens'], effective['duration_s']
    period = effective['monitor_period_s'] or json.loads((Path(campaign)/'manifest.json').read_text('utf-8'))['configuration']['monitor_period_s'] or window
    delta = run['margin'] * (long-short)
    midpoint = (short+long)/2
    scheduler = expected_scheduler(row['policy'])
    trace = json.loads((Path(campaign) / 'inputs/trace.json').read_text('utf-8'))
    def input_type(t):
        return trace_label(trace, effective['trace'], t, duration)
    ...
    check(all(0 <= ticks(r['arrival_s']) < ticks(duration) for r in requests.values()), 'Arrival bounds')
    ...
    for a in result['attempts']:
        ...
        if a['start_s'] is not None:
            ...
            if a['stage'] == 'PE':
                ...
                check(p['actual_input'] == original_input + g and p['actual_remaining'] == remaining, 'PE reprefill request')
                planned_ticks = ticks(p['ttft_s']) + (remaining - 1) * ticks(p['tpot_s'])
                check(ticks(a['planned_service_s']) == planned_ticks, 'PE remaining-token service formula')
                if a['exit_reason'] == 'completed':
                    check(ticks(a['executed_service_s']) == planned_ticks, 'Completed PE service')
                else:
    ...
    last, number, active, transition = 0, 0, (long if run['initial_deployment'] == 'target' else short), None
    flip_begin, flip_end = {}, {}
    max_wait = Counter()
    ...
    with gzip.open(directory / 'events.jsonl.gz', 'rt', encoding='utf-8') as stream:
        for line in stream:
            ...
            dt, observed_dt = now - last, max(0, min(now, ticks(duration)) - min(last, ticks(duration)))
            ...
            if kind == 'instance_created':
                ...
                check(now % ticks(period) == 0, 'Monitor period')
                ...
                desired = long if active == short and mean is not None and mean > midpoint + delta else short if active == long and mean is not None and mean < midpoint - delta else active
    ...
    for f in result['flips']:
        ...
        flips.append({'run_id': rid, 'flip_id': f['flip_id'], 'direction': f['direction'],
            ...
            'groups': len(f['groups']), 'in_drain': detected > ticks(duration),
