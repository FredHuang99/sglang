...
def pack_transfer_metadata(manifest: dict, scalar_fields: dict) -> bytes:
    ...
    for key, value in scalar_fields.items():
        ...
        descriptors.append((1, value_kind, key_offset, key_length, value_offset, value_length))
...
def unpack_transfer_metadata(blob: bytes) -> tuple[dict, dict]:
    ...
    for idx in range(num_records):
        start = _META_HEADER_SIZE + idx * _META_DESC_SIZE
        section_kind, value_kind, _, key_offset, key_length, value_offset, value_length, _ = _META_DESC.unpack(
            blob[start : start + _META_DESC_SIZE]
        )
