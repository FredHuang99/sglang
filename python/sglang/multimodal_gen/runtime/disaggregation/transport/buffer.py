...
class TransferTensorBuffer:
    ...
    def __init__(
        ...
        self._shared_region: SharedMemoryRegion | None = None

        if device == "cpu":
            if shared_memory_backing is None:
                shared_memory_backing = True
            if shared_memory_backing:
                self._shared_region = SharedMemoryRegion(
                    role_name,
                    "tensor",
                    actual_size,
    ...
    def cleanup(self) -> None:
        if self._shared_region is not None:
            self._shared_region.cleanup(logger)
    ...
    def write_tensor(
        ...
        if stream is not None:
            ...
            dst.copy_(src_bytes, non_blocking=src_tensor.device.type != "cpu")
    ...
    def read_tensor(
        ...
        return src.to(device, non_blocking=_device_type(device) != "cpu")
...
class TransferMetaBuffer:
    ...
    def __init__(
        ...
        *,
        ...
        self._shared_region: SharedMemoryRegion | None = None
        if shared_memory_backing:
            self._shared_region = SharedMemoryRegion(role_name, "meta", total_size)
            self._pool = self._shared_region.tensor
        else:
