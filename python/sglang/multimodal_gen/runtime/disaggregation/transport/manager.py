...
class DiffusionTransferManager:
    ...
    def start_background_loops(
        ...
        *,
        ...
        if start_send_loop:
            queue_count, total_workers, worker_counts = self._resolve_send_runtime_config(
                self._send_concurrency
            )
    ...
    def stage_tensors_async(
        ...
        if total_size > 0:
            ...
            if data_slot is None:
                ...
                return None, None
            manifest = self._buffer.write_tensors_from_gpu(data_slot, tensor_fields, stream)
        ...
        if meta_slot is None:
            ...
            logger.warning("TransferManager: failed to allocate meta slot for %s", request_id)
            return None, None

        try:
            meta_size = self._meta_buffer.write_metadata(meta_slot, manifest, scalar_fields)
        except Exception:
            if data_slot is not None:
                self._buffer.free(data_slot)
    ...
    def _load_received_transfer(
        ...
        if pending is None:
            raise ValueError(f"TransferManager: no pending receive slot for {request_id}")
    ...
    def push_to_peer(
        ...
        ret = self._engine.transfer_sync(dest_session_id, src_addr, dest_addr, transfer_size)
        return ret == 0
    ...
    def allocate_receive_slot(
        ...
        if meta_slot is None:
            ...
            logger.warning("TransferManager: failed to allocate receive meta slot for %s", request_id)
            return None
    ...
    def send_direct_message(self, endpoint: str, msg) -> None:
        if self._context is None:
            raise RuntimeError("TransferManager direct-control context is not initialized")
        if not endpoint:
    ...
    def _register_peer_send(self, msg: dict) -> None:
        ...
        with self._lock:
            ...
            if request_id in self._aborted_requests:
                ...
                return
            if request_id in self._terminal_send_states:
                logger.debug("TransferManager: ignoring duplicate peer info for terminal %s", request_id)
                return
    ...
    def _submit_send_task(self, request_id: str, queue_idx: int | None = None) -> None:
        with self._lock:
            ...
            if peer_info is None:
                logger.debug("TransferManager send loop: missing peer info for %s", request_id)
                return
    ...
    def _execute_send(
        ...
        try:
            ...
            if staged.meta_slot is not None and peer_info.meta_transfer_size > 0:
                src_addrs.append(self._meta_buffer.pool_data_ptr + staged.meta_slot.offset)
    ...
    def _receive_loop(self) -> None:
        while self._running and self._control_pull is not None:
            ...
            try:
                ...
                logger.exception("TransferManager receive loop failed to decode message")
                continue
            ...
            if msg_type == TransferMsgType.PEER_INFO:
                ...
                logger.warning("TransferManager receive loop: unsupported msg_type=%s", msg_type)
