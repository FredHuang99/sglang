...
from sglang.multimodal_gen.runtime.disaggregation.transport.shared_memory import (
    SharedMemoryAttachment,
    copy_to_shared_memory,
    get_shared_memory_attachment,
)
...
class DiffusionTransferManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._shared_memory_attachments: dict[str, SharedMemoryAttachment] = {}
    ...
    def _get_shared_memory_attachment(self, name: str) -> SharedMemoryAttachment:
        return get_shared_memory_attachment(name, self._shared_memory_attachments)
    ...
    def _local_copy(
        ...
        return copy_to_shared_memory(
            src_addr=src_addr,
            dst_shm_name=dst_shm_name,
            dst_shm_offset=dst_shm_offset,
            length=length,
            attachments=self._shared_memory_attachments,
            logger=logger,
        )
    ...
    def stage_tensors_async(
        ...
        """Stage tensors and metadata without blocking the scheduler loop."""
    ...
    def _register_peer_send(self, msg: dict) -> None:
        request_id = require_request_id(
            msg,
            context="TransferManager peer-info",
            logger=logger,
        )
        ...
        while self._running and self._control_pull is not None:
            ...
            if msg_type == TransferMsgType.PEER_INFO:
                ...
                if (
                    require_request_id(
                        msg,
                        context="TransferManager ready message",
                        logger=logger,
                    )
                    is None
                ):
                ...
                if (
                    require_request_id(
                        msg,
                        context="TransferManager failed message",
                        logger=logger,
                    )
                    is None
                ):
                ...
                if (
                    require_request_id(
                        msg,
                        context="TransferManager abort message",
                        logger=logger,
                    )
                    is None
                ):
                ...
                logger.warning(
                    "TransferManager receive loop: unsupported msg_type=%s msg=%s",
                    msg_type,
                    msg,
                )
