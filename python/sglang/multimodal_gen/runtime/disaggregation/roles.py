...
def filter_modules_for_role(
    ...
    role: "RoleType",
    encoder_required_modules: list[str] | None = None,
) -> list[str]:
    ...
    - ENCODER: encoder modules + shared + explicitly declared extra modules
    ...
    encoder_required = set(encoder_required_modules or [])
    filtered = []
    for name in module_names:
        ...
        if module_role is None:
            ...
            filtered.append(name)
        elif role == RoleType.ENCODER and name in encoder_required:
            # Some encoder stages depend on modules that are otherwise classified
            # as decoder-only, such as VAE-based image encoding.
            filtered.append(name)
