...
class Hunyuan3DShapeBeforeDenoisingStage(PipelineStage):
    ...
    def _resolve_runtime_dtype(self, sample_tensor: torch.Tensor | None = None) -> torch.dtype:
        ...
        buffers_fn = getattr(self.conditioner, "buffers", None)
        if callable(buffers_fn):
            try:
                for buffer in buffers_fn():
                    if isinstance(buffer, torch.Tensor) and torch.is_floating_point(buffer):
                        return buffer.dtype
            except TypeError:
                pass
    ...
    def forward(self, batch: Req, server_args: ServerArgs) -> Req:
        ...
        device = self.device
        dtype = self._resolve_runtime_dtype(image if isinstance(image, torch.Tensor) else None)
