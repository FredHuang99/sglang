...
class TestFilterModulesForRole(unittest.TestCase):
    ...
    def test_encoder_skips_transformer_and_vae_by_default(self):
    ...
    def test_encoder_can_include_explicit_vae_dependency(self):
        result = filter_modules_for_role(
            self.WAN_MODULES,
            RoleType.ENCODER,
            encoder_required_modules=["vae"],
        )
...
class TestFilterModulesLTX2(unittest.TestCase):
    ...
    def test_encoder_includes_explicit_audio_vae_dependency(self):
        result = filter_modules_for_role(
            self.LTX2_MODULES,
            RoleType.ENCODER,
            encoder_required_modules=["audio_vae"],
        )
        self.assertIn("connectors", result)
        self.assertIn("text_encoder", result)
        self.assertIn("audio_vae", result)
        self.assertNotIn("vae", result)
        self.assertNotIn("vocoder", result)
        self.assertNotIn("transformer", result)
